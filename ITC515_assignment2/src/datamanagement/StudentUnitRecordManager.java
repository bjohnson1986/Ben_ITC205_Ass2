	...
	private IStudentUnitRecord setStudentUnitRecord(Integer subjectCode, String studentIdentification) {
		...
		//Boolean flag to check if current element has the same studentID and subjectCode in the XML database.
			...
			if (isEqualToStudentAndSubject) {
				newStudentPerUnit = new StudentUnitRecord(new Integer(element.getAttributeValue("sid")), element.getAttributeValue("uid"),
						new Float(element.getAttributeValue("asg1")).floatValue(), new Float(
	...
	public StudentUnitRecordList getRecordsByUnit(String subjectCode) {
		...
		for (Element element : (List<Element>) XmlManager.getXML().getDocument().getRootElement().getChild("studentUnitRecordTable").getChildren("record")) {
			if (subjectCode.equals(element.getAttributeValue("uid")))
				unitRecord.add(new StudentUnitRecordProxy(new Integer(element.getAttributeValue("sid")), element.getAttributeValue("uid")));
		}
		if (unitRecord.size() > 0)
			subjectToStudentRecord__.put(subjectCode, unitRecord); // be careful - this could be empty
		return unitRecord;
	}
